...
### P1（Agar 向扩展）
- [ ] 分裂/合并（Split/Merge）里程碑（拆分任务）
- [ ] 分裂：输入 + 服务端指令（如 Space => split），验收：一次按键一次分裂；有频率限制；不满足条件时忽略
- [ ] 分裂：质量守恒 + 子球生成，验收：总面积守恒（允许极小误差）；子球继承颜色/昵称；id 稳定
- [ ] 分裂：冲量/速度调参，验收：子球按方向冲出并逐渐减速；不会穿墙/越界
- [ ] 合并：按 cell 的合并计时，验收：`mergeDelayMs` 前绝不合并；到期且重叠时可合并
- [ ] 碰撞规则：分裂后交互正确，验收：吃豆/吃人规则与单球一致；不会出现重复计分/重复吞噬
- [ ] UI/反馈：冷却/状态提示，验收：提示“可分裂/剩余冷却/可合并倒计时（可选）”，且不明显影响性能
- [ ] 可配置：分裂/合并参数可调，验收：通过 config.json（必要时 env）可调整行为，无需改代码
- [ ] 喷射质量
- [ ] 刺球/危险物（virus 等）
- [ ] 皮肤 + 基础滥用防护（昵称过滤等）
